...
const inputCls =
  "w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#2eb781]/20 focus:border-[#2eb781] transition-all text-sm text-gray-900 placeholder-gray-400";

function getContractLabel(val: string | null) {
  return (
    CONTRACT_TYPES.find((c) => c.value === val)?.label || "Full-time Staff"
  );
}
...
export default function StaffsClient({
  ...
  async function handleDelete(s: Staff) {
    if (
      !confirm(
        `Remove "${s.full_name}" from your company? Their login access will also be revoked.`,
      )
    )
      return;
    setDeletingId(s.id);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage all staff members in your company.
          </p>
        </div>
        <button
          onClick={() => {
            ...
            <form action={handleCreate} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">
                    <span className="flex items-center gap-1.5">
                      <Briefcase className="w-3.5 h-3.5" /> Contract Type
                    </span>
                  </label>
              ...
              <label className="flex items-start gap-3 p-4 bg-purple-50 border border-purple-100 rounded-xl cursor-pointer hover:bg-purple-100/60 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-purple-700/80 mt-0.5">
                    This person can log in and manage organizations, staff,
                  ...
                  {editingId === s.id ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => handleUpdate(s.id, s.isOwner)}
                    ...
                    <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-4 items-center">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${roleName ? "bg-[#2eb781]/10 text-[#2eb781]" : "bg-gray-100 text-gray-500"}`}
                          >
                        ...
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${s.contract_type === "contract" ? "bg-amber-50 text-amber-700" : "bg-blue-50 text-blue-700"}`}
                          >
