...
export default function GoogleIntegrationClient({
  ...
  const handleConnect = (companyId: string) => {
    ...
    window.open(
      `/api/integrations/google/connect?companyId=${companyId}`,
      "Connect Google",
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Connect your Google account for Calendar and Meet features.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden p-6 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-8 border-b border-gray-100">
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              {integration ? (
                <span className="inline-flex items-center gap-1 w-fit rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-[#2eb781] ring-1 ring-inset ring-[#2eb781]/20">
            ...
            {integration ? (
              ...
              <p className="text-sm text-gray-500 max-w-lg">
                By connecting your Google account, you allow us to create events
                on your calendar and generate Google Meet links for your
                scheduled calls automatically.
              </p>
            )}
          </div>
          ...
          <div className="shrink-0 flex items-center gap-3">
            {integration ? (
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-[#2eb781] text-white rounded-xl text-sm font-semibold hover:bg-[#2eb781]/90 transition-colors shadow-sm disabled:opacity-50"
              >
                {isConnecting ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Calendar Feature */}
            <div
              className={`p-5 rounded-2xl border ${integration ? "border-gray-200 bg-white" : "border-gray-100 bg-gray-50"} flex flex-col gap-4`}
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border bg-blue-50 border-blue-200">
                  <Calendar
                    className={`w-6 h-6 ${integration ? "text-blue-600" : "text-blue-400"}`}
                  />
                </div>
                <div>
                  <h3
                    className={`font-semibold text-lg ${integration ? "text-gray-900" : "text-gray-500"}`}
                  >
                    Google Calendar
                  </h3>
                  <p
                    className={`text-sm mt-1 ${integration ? "text-gray-500" : "text-gray-400"}`}
                  >
                    Sync your scheduled events and meetings seamlessly with your
              ...
              <div className="mt-auto pt-2 flex items-center justify-between">
                <span
                  className={`text-sm font-medium ${integration ? "text-green-600" : "text-gray-400"}`}
                >
            ...
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border bg-green-50 border-green-200">
                  <Video
                    className={`w-6 h-6 ${integration ? "text-green-600" : "text-green-400"}`}
                  />
                </div>
                <div>
                  <h3
                  ...
                  <p
                    className={`text-sm mt-1 ${integration ? "text-gray-500" : "text-gray-400"}`}
                  >
                    Quickly generate video conferencing links for meetings and
            ...
            <div className="p-5 space-y-4">
              {!integration && (
                <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-xl text-sm text-yellow-800">
                  Warning: You are not connected to Google. Link generation is
                  currently mocked.
                </div>
              )}
