...
export default function TaskDetailsSidebar({
  ...
  const [collaborators, setCollaborators] = useState<
    { id: string; staff_id: string; staffs: StaffMember }[]
  >([]);
  ...
  const fetchCollaborators = async () => {
    ...
    try {
      const { data, error } = await getTaskCollaborators(task.id);
      ...
      const formatted = data
        .filter((c: CollaboratorRow) => c.staffs)
        .map((c: CollaboratorRow) => {
          const staff = c.staffs;
  ...
  const handleAddCollaborator = async () => {
    if (!selectedCollaboratorId || !task) return;
    setIsAddingCollaborator(true);
    const result = await addTaskCollaborator(task.id, selectedCollaboratorId);
    if (result.error) {
  ...
  const handleRemoveCollaborator = async (staffId: string) => {
    if (!task) return;
    const result = await removeTaskCollaborator(task.id, staffId);
    if (result.error) {
      toast.error(result.error);
    } else {
      ...
      >
        ...
        <div className="flex-1 overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-12 h-full">
                ...
                {task.is_project_task ? (
                  ...
                  <div className="py-12 border-2 border-dashed border-gray-100 rounded-3xl flex flex-col items-center justify-center text-center p-10 bg-gray-50/30">
                    ...
                    <p className="text-xs text-gray-500 mt-2 max-w-[200px] leading-relaxed">
                      This is an internal organizational task. Use the Project
                      view for advanced features like sub-tasks and priority
                      management.
                    </p>
                  </div>
                )}
            ...
            <div className="lg:col-span-4 bg-gray-50/30 p-8 space-y-6 border-l border-gray-50 overflow-y-auto no-scrollbar">
              {/* Status */}
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-gray-400">
                  ...
                  {loadingCollaborators ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {collaborators.map((c) => (
                        ...
                        >
                          <div className="w-5 h-5 rounded-md bg-violet-100 text-violet-700 flex items-center justify-center text-[9px] font-bold">
                            {(c.staffs?.full_name || "?")[0].toUpperCase()}
                          </div>

                          <span className="text-xs font-bold text-gray-700">
                            {c.staffs?.full_name || "Unknown"}
                          </span>

                          <button
                            type="button"
                            onClick={() => handleRemoveCollaborator(c.staff_id)}
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        <div className="relative flex-1">
                          ...
                          <select
                            ...
                            className="w-full pl-8 pr-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#2eb781]/20 focus:border-[#2eb781] transition-all appearance-none cursor-pointer"
                          >
                        ...
                        <button
                          ...
                          className="shrink-0 flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[#2eb781] text-white text-xs font-bold hover:bg-[#279e6f] transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        >
        ...
        <div className="px-8 py-5 border-t border-gray-100 bg-white/80 backdrop-blur-sm flex items-center justify-between">
          <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
            <Loader2
              className={`w-3 h-3 text-emerald-500 ${isAddingSubTask || loadingSubTasks ? "animate-spin" : ""}`}
            />
