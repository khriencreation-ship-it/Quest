      ...
      <aside className="w-24 bg-gray-900 border-r border-gray-800 flex-col items-center py-6 fixed inset-y-0 left-0 z-50 overflow-y-auto hidden md:flex">
        ...
        {isManager && (
          <>
            <Link
              href="/dashboard"
              className="flex flex-col items-center gap-2 w-full px-2 mb-6 group"
            >
              <div
                className={`w-12 h-12 rounded-2xl text-white flex items-center justify-center text-lg font-bold shadow-sm transition-all cursor-pointer ${
                  isCompanyActive
                    ? "bg-[#2eb781] scale-105"
                    : "bg-gray-800 border border-gray-700 hover:bg-gray-700 group-hover:scale-105"
                }`}
              >
          ...
          {departments.map((org) => {
            ...
            return (
              <Link
                ...
                className="flex flex-col items-center gap-2 w-full px-2 group"
              >
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold shadow-sm transition-all cursor-pointer ${
                    isOrgActive
                      ? "bg-[#2eb781] text-white scale-105"
                      : "bg-gray-800 border border-gray-700 text-gray-300 hover:bg-gray-700 group-hover:scale-105"
                  }`}
                >
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col fixed inset-y-0 left-0 md:left-24 z-40">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1">
          {activeMenus.map((item) => {
            ...
            const targetHref = activeOrgId
              ? `${item.href}?org=${activeOrgId}`
              : item.href;
            const isActive =
              pathname === item.href ||
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-5 h-5 ${isActive ? "text-[#2eb781]" : "text-gray-400"}`}
                />
                {item.name}
              </Link>
            );
