...
type ProjectDetailClientProps = {
  ...
  serviceType?: string;
  projectStaff?: any[];
  /** Can assign tasks, see all members' tasks, and move any task between
   *  kanban columns — ONLY the designated department manager. */
...
export default function ProjectDetailClient({
  ...
  const activities = (() => {
    ...
    tasks.forEach((task: any) => {
      ...
      events.push({
        ...
        actor:
          projectStaff?.find((s) => s.user_id === task.created_by)?.full_name ||
          "System",
      });
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex space-x-2">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <tab.icon
                    className={`w-4 h-4 ${isActive ? "text-[#2eb781]" : "text-gray-400"}`}
                  />
                  {tab.label}
                  {isActive && (
      ...
      {/* Tab Contents */}
      <div
        className={`bg-white rounded-xl border border-gray-100 shadow-sm min-h-[500px] ${activeTab === "tasks" || activeTab === "documents" ? "p-0" : "p-6"}`}
      >
        {activeTab === "overview" && (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white rounded-2xl p-4 border-2 border-gray-100">
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 text-center">
                      Start Date
                    </h4>
                ...
                {activities.length > 0 ? (
                    ...
                    {activities.slice(0, 5).map((activity) => (
                      ...
                      >
                        <div className="flex items-center">
                          <div
                            className={`flex flex-col items-center justify-center w-8 h-8 rounded-full border-2 border-white shadow-sm ring-1 z-10 transition-transform group-hover:scale-110 ${
                  ...
                  <div className="border-2 border-dashed border-gray-100 rounded-3xl flex flex-col items-center justify-center text-center p-10 bg-gray-50/30">
                    ...
                    <p className="text-sm text-gray-500 mt-1 max-w-[180px] leading-relaxed">
                      Create and complete tasks to see your project's heartbeat
                      here.
                    </p>
                  </div>
                )}
          ...
          <div className="animate-in fade-in duration-300">
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Customize and save the specific scope configuration for this
                project.
              </p>
            </div>
              ...
              {serviceType === "social_media" && (
                <SocialMediaScope
                  ...
                  onSave={async (config: any) =>
                    updateProjectScope(project.id, config)
                  }
                />
              )}
          ...
          <div className="animate-in fade-in duration-300 h-full flex flex-col">
            <div className="mb-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Generated automatically based on the `{project.services?.name}
                  ` scope rules.
                </p>
              </div>
            ...
            <div className="flex-1 bg-white border border-gray-200 rounded-xl overflow-hidden mt-6">
              <div className="grid grid-cols-7 border-b border-gray-200 bg-gray-50 text-xs font-semibold text-gray-400">
                {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                  (day) => (
                    <div
                      key={day}
                      className="p-3 text-center uppercase tracking-wider border-r border-gray-200 last:border-0"
                    >
                      {day}
                    </div>
                  ),
              ...
              <div className="grid grid-cols-7 bg-gray-50/50 gap-px auto-rows-[120px]">
                {Array.from({ length: 35 }).map((_, i) => {
                  ...
                  const postsToday =
                    scopeConfig?.posting_schedule?.filter(
                      (s: any) => s.day === fullDayName,
                    ) || [];

                  // Check report day (naive)
                  const reportFrequency =
                    scopeConfig?.reporting?.frequency || "Monthly";
                  const reportDay =
                    scopeConfig?.reporting?.report_day || "Friday";
                  const isReportDay =
                    (reportFrequency === "Weekly" &&
                      fullDayName === reportDay) ||
                    (reportFrequency === "Monthly" &&
                      fullDayName === reportDay &&
                      dateNum <= 31 &&
                      ((scopeConfig?.reporting?.week_of_month === "1st" &&
                        dateNum <= 7) ||
                        (scopeConfig?.reporting?.week_of_month === "2nd" &&
                          dateNum > 7 &&
                          dateNum <= 14) ||
                        (scopeConfig?.reporting?.week_of_month === "3rd" &&
                          dateNum > 14 &&
                          dateNum <= 21) ||
                        ((!scopeConfig?.reporting?.week_of_month ||
                          scopeConfig?.reporting?.week_of_month === "4th") &&
                          dateNum > 21 &&
                          dateNum <= 28) ||
                        (scopeConfig?.reporting?.week_of_month === "Last" &&
                          dateNum + 7 > 31)));

                  // Ad runtime (just visual)
                  const isAdRunning =
                    scopeConfig?.ads?.enabled && i > 5 && i < 25;

                  return (
                    <div
                      key={i}
                      className="bg-white p-2 relative group hover:bg-gray-50 transition-colors"
                    >
                      <span
                        className={`text-xs font-semibold ${dateNum > 31 ? "text-gray-300" : "text-gray-500"}`}
                      >
                      ...
                      <div className="mt-1 space-y-1">
                        {isAdRunning && (
                          <div
                            className="absolute top-0 inset-x-0 h-1 bg-yellow-400/20"
                            title="Ads Running"
                          />
                        )}
                        ...
                        {postsToday.map((post: any, idx: number) => (
                          <div key={idx} className="space-y-1">
                            {post.content_types?.map(
                              (ct: string, ctIdx: number) => (
                                <div
                                  key={ctIdx}
                                  className="px-1.5 py-1 bg-emerald-50 border border-emerald-100 rounded text-[10px] font-bold text-emerald-700 truncate"
                                  title="Format"
                                >
