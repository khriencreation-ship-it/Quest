...
} from "@/app/actions/organization_members";
...
export default function ManageStaffSection({
  ...
  async function handleAddStaff(e: React.FormEvent) {
    ...
    const result = await addStaffToOrganization(
      organizationId,
      selectedStaffId,
    );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {isGeneral
                ? "Everyone in the company is automatically a member of the General department."
          ...
          <form onSubmit={handleAddStaff} className="mt-6 flex gap-3">
            ...
            >
              ...
              {availableStaff.length === 0 && (
                <option value="" disabled className="text-gray-900">
                  All company staff are already in this department.
                </option>
              )}
            </select>
            <button
      ...
      <div className="divide-y divide-gray-100">
        {members.length === 0 ? (
          <div className="p-8 text-center text-gray-500 text-sm">
            No members found in this department. The owner has access by
            default.
          </div>
        ) : (
          members.map((member) => (
            <div
              key={member.id}
              className="p-4 sm:px-8 flex items-center justify-between hover:bg-gray-50 transition-colors"
            >
              ...
              {!isGeneral && (
                <button
                  onClick={() =>
                    handleRemoveStaff(member.id, member.full_name || "User")
                  }
                  disabled={isLoading}
