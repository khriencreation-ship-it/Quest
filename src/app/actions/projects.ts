...
export async function updateProject(formData: FormData) {
  ...
  try {
    ...
    const removedStaffIds = currentStaffIds.filter(
    ...
    if (removedStaffIds.length > 0) {
      for (const rStaffId of removedStaffIds) {
        const rStaff = currentProjectStaff?.find(
          (s) => s.staff_id === rStaffId,
        );
        const rStaffUserId = rStaff?.staffs?.[0]?.user_id;
        const rStaffName = rStaff?.staffs?.[0]?.full_name || "Staff";
...
export async function updateProjectScope(projectId: string, scope_config: any) {
  ...
  try {
    ...
    if (!project.service_id) {
      return {
        error:
          "This project does not have a service assigned. Please assign a service first.",
      };
    }

    // 2. Upsert into client_scope (as per the new SQL schema)
    const { error: upsertError } = await supabase.from("client_scope").upsert(
      {
    ...
    if (upsertError) {
      ...
      if (upsertError.code === "42P01") {
        console.warn(
          "client_scope table not found. Falling back to project.scope_config only.",
        );
      } else {
