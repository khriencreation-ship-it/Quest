...
export default async function StaffsPage() {
  ...
  const { data: staffsRaw } = await supabase
    .from("staffs")
    .select(
      "id, user_id, first_name, last_name, full_name, email, phone, role_id, contract_type, is_manager, created_at",
    )
  ...
  const ownerAlreadyInList = staffList.some(
    (s) => s.user_id === userData!.user.id,
  );
  ...
  if (!ownerAlreadyInList) {
    ...
    const ownerName =
      userData!.user.user_metadata?.full_name || ownerEmail.split("@")[0];
    const ownerEntry = {
