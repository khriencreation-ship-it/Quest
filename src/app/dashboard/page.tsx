...
export default async function DashboardHome(props: {
  ...
  if (orgId) {
    ...
    const { data: org } = await adminSupabase
      ...
      .eq("id", orgId)
      .single();
    contextName = org?.name || "Unknown Department";
  } else {
  ...
  if (assignedTaskIds && assignedTaskIds.length > 0) {
    const { data: pTasks } = await adminClient
      ...
      .in(
        "id",
        assignedTaskIds.map((a) => a.task_id),
      )
      .neq("status", "done")
  ...
  const myTasksCombined = [
    ...myProjectTasks.map((t) => ({
      ...t,
      type: "Project",
      context: t.projects?.name,
    })),
    ...myOrgTasks.map((t) => ({
      ...t,
      type: "Internal",
      context: t.organizations?.name,
    })),
  ]
    .sort(
      (a, b) =>
  ...
  if (orgId) {
    ...
    if (company) {
      ...
      recentDepartments = topDepartments || [];
    ...
    <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">
          Here is what is happening with your {orgId ? "workspace" : "company"}{" "}
          today.
        </p>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              ...
              <Link
                ...
                className="text-sm font-bold text-[#2eb781] hover:text-[#279e6f]"
              >
                View All Tasks
              </Link>
            </div>
              ...
              {myTasksCombined.length > 0 ? (
                <div className="divide-y divide-gray-50">
                  {myTasksCombined.map((task: any) => (
                    <div
                      key={task.id}
                      className="p-4 hover:bg-gray-50 transition-colors rounded-xl group/task"
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0 pr-4">
                          ...
                          <h3 className="font-bold text-gray-900 group-hover/task:text-[#2eb781] transition-colors">
                            {task.title}
                          </h3>
                          {task.due_date && (
                ...
                <div className="p-12 text-center">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    You don't have any active tasks assigned to you right now.
                  </p>
                </div>
              )}
            </div>
              ...
              {recentProjects.length > 0 ? (
                  ...
                  {recentProjects.map((project: any) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div
                          className={`w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center group-hover:bg-[#2eb781]/10 transition-colors`}
                        >
                ...
                <div className="p-12 text-center border-2 border-dashed border-gray-100 rounded-2xl">
                  <p className="text-sm text-gray-500">
                    No active projects to display in this context.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Link
                    ...
                    className="text-xs font-bold text-[#2eb781]"
                  >
                    View All
                  </Link>
                </div>
                <div className="space-y-4">
                  {recentDepartments.map((dpt: any) => (
                    <Link
                      key={dpt.id}
                      href={`/dashboard?org=${dpt.id}`}
                    ...
                    >
                      <div className="w-10 h-10 rounded-lg bg-gray-900 text-white flex items-center justify-center font-bold text-xs shrink-0 group-hover:bg-[#2eb781] transition-colors">
                        {dpt.name.substring(0, 2).toUpperCase()}
                      </div>
            ...
            <div className="bg-[#2eb781] rounded-3xl p-8 text-white shadow-xl shadow-[#2eb781]/20 relative overflow-hidden group">
              <div className="relative z-10">
                ...
                <p className="text-emerald-50 text-sm leading-relaxed mb-6 opacity-90">
                  If you have questions about your assigned tasks or project
                  deadlines, contact your manager or the project lead.
                </p>
